...
;; Cât timp reprezentăm stivele ca liste Racket, o rotație va avea complexitate O(n),
   ...
   if (< (queue-size-l q) (+ (queue-size-r q) 1))
      (
       struct-copy queue q
                          [
                           left
                           (rotate (queue-left q) (append (list x) (queue-right q)) empty-stream)
                          ]
                          [
                           size-l
   ...
   if (< (- (queue-size-l q) 1) (queue-size-r q))
      (
       struct-copy queue q
                          [
                           left
                           (stream-rest (rotate (queue-left q) (queue-right q) empty-stream))
                          ]
                          [
                           size-l
                           (- (+ (queue-size-l q) (queue-size-r q)) 1)
