...
(define (dequeue q)
  (if (null? (queue-left q))
      (struct-copy queue q [
                            left
                            (cdr (muta-din-right-in-left (queue-left q) (queue-right q)))
                           ]
                           [
                            size-l
                            (- (+ (queue-size-l q) (queue-size-r q)) 1)
                           ]
                           [
                            right
                            null
...
(define (top q)
  (if (null? (queue-left q))
      (car (muta-din-right-in-left (queue-left q) (queue-right q)))
      (car (queue-left q))
  )
)


(define (muta-din-right-in-left noul-left right)
  (if (null? right)
      noul-left
      (
       muta-din-right-in-left (append (list (car right)) noul-left) (cdr right)
      )
  )
)
